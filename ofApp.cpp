...
void Actor::update(const int& frame_span, vector<glm::vec3>& location_list, vector<vector<int>>& next_index_list, vector<int>& destination_list) {
	...
	while (this->log.size() > 250) { this->log.pop_back(); }
}
...
void ofApp::setup() {
	...
	for (int i = 0; i < 6; i++) {
		...
		this->actor_list.back()->setColor(color_palette[i]);
	}
}
...
void ofApp::draw() {
	...
	this->cam.begin();
	ofRotateY(ofGetFrameNum() * 1.3333333333333333);
	...
	for (auto& actor : this->actor_list) {
		...
		ofFill();
		ofDrawSphere(actor->getLocation(), 10);
